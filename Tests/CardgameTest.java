...
public class CardgameTest {
    ...
    private static int playersdrawn = 0;
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<players.get_players().length;i++) {
            System.out.print("Player "+ players.get_players()[i].getDenom() + "'s initial hand is ");
            for (int x = 0; x < 4; x++) {
                System.out.print(players.get_players()[i].gethand()[x] + " ");
            }
            System.out.println();
    ...
    class Player extends Thread {
        ...
        public synchronized void atomicAction(){
            ...
            synchronized(owndeck){
                int index = findcard();
                int draw = owndeck.getcards()[0];

                reorderowndeck();
                System.out.println("Player "+ prefDenom + " has picked up a "+ draw + " from deck " +prefDenom);
                discard = edithand(draw, index);
                // Draws the needed card and reorders the deck to be ready to be drawn again
                //player1 needs to wait for player 2 to be finished drawing
            }
            synchronized (playerlock) {
                playersdrawn ++;
                if (playersdrawn < number){
                    try {
                        playerlock.wait();
    ...
    synchronized (disdeck) {
        disdeck.addcard(discard, 3);
        System.out.println("Player "+ prefDenom + " has discarded a "+ discard +" to deck " + ((prefDenom%number)+1));
        // Discard card is given to the right deck
    }

            System.out.print("Player " + prefDenom + "'s current hand is ");
            for (int i = 0; i < 4; i++) {
                System.out.print(gethand()[i] + " ");
            }
            System.out.println();
        ...
        public int edithand(int card, int index){
            ...
            try {
                ...
                System.out.println("Error: problem indexing into the hand");
                e.printStackTrace();
            }
            return oldcard;
        }
        ...
        public int findcard(){
            ...
            for (int i = 0; i <4; i++){
                int index = (nextcard + i) %4;
                if(hand[index] != getDenom()){
                    nextcard = (index + 1) %4;
                    return index;
                }
            }
        ...
        public void reorderowndeck(){
            owndeck.addcard(owndeck.getcards()[1],0);
            owndeck.addcard(owndeck.getcards()[2],1);
            owndeck.addcard(owndeck.getcards()[3],2);
            owndeck.addcard(-1,3);
    ...
    class Deck{
        ...
        public synchronized void addcard(int card, int index){
             try {
                cards[index] = card;
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Error: problem indexing into the deck");
        ...
        public synchronized int[] getcards(){
            return cards.clone();
        }
    ...
    class PlayerList {
        ...
        public PlayerList(){
            ...
            for(int i = 1; i<=number; i++){
                players[i-1].setowndeck(decks[i-1]);
                players[i-1].setdisdeck(decks[(i)%number]);
            }
        }
    ...
    class CardClass{
        ...
        public int[] setPack(String location, int[] pack) {
            try (BufferedReader file = new BufferedReader(new FileReader(location))){
                ...
                System.out.println("Error: File not found at location. Please try again.");
                e.printStackTrace();
                System.exit(1);
            } catch (IOException e) {
                System.out.println("Error: there was an IOException in reading the line. Please try again.");
                e.printStackTrace();
                System.exit(1);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Error: Index out of bounds exception. Please try again.");
                e.printStackTrace();
                System.exit(1);
            }
            return pack;
    ...
    public String checkName(String location) {
        ...
        if (location.length() >= 4){
            ...
            if (location.contains("..")){
                throw new IllegalArgumentException("Invalid filename: you are not allowed to use '..' for security reasons.\nPlease restart."); 
            }
    ...
    public void dealing(int[] pack, PlayerList players){
        ...
        for(int card = 0; card < 4 ; card++){
            for(int player = 0; player < number ; player ++){
                (players.get_players()[player]).addcard_hand(pack[player+card*number],card);
            }
        }
        int offset = (pack.length/2);
        for(int card = 0; card < 4 ; card++){
            for(int player = 0 ;  player < number ; player ++){
                (players.get_decks()[player]).addcard(pack[offset+player+card*number],card);
            }
        }
    }
}